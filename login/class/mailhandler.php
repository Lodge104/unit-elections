    ...
    public function sendMail($userarr, $type)
    {
        ...
        if ($type == 'Verify') {
            ...
            foreach ($userarr as $usr) {
                ...
                $mail->addAddress($usr['email'], $usr['username']);
				$mail->AddReplyTo($usr['email'], $usr['username']);

                if ($this->admin_verify == 'true') {
                $mail->AddBCC('survey@lodge104.com', $usr['username']);
                }
            }
            ...
            //Set the subject line
            $mail->Subject = $this->site_name . ' Account Activated!';
